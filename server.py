            ...
            if r.status_code == 200:
                ...
                if choices:
                    ...
                    if text:
                        k["fails"]     = 0
                        k["last_used"] = time.time()
                        log_srv("[OPENROUTER] Success model=" + OPENROUTER_MODEL)
                        return text
            elif r.status_code in (401, 403):
                k["dead"]  = True
                k["fails"] = 99
                log_srv("[OPENROUTER] Key " + k["name"] + " auth error - dead")
            elif r.status_code == 429:
                k["fails"] += 1
...
# ---------------------------------------------------------------------------
#  COLOR MAP - maps color names to RGB float tuples
# ---------------------------------------------------------------------------
COLOR_MAP = {
    "red":       (1.0, 0.0, 0.0),
    "green":     (0.0, 0.8, 0.0),
...
def clean_poisoned_cache():
    """Remove cached GLB files generated by the fallback.
    These are identified by the AI3DStudio-Fallback generator tag."""
    try:
        ...
        for fname in os.listdir(CACHE_DIR):
            ...
            try:
                with open(fp, "rb") as fh:
                    ...
                    fh.read(4)  # chunk type
                    json_data = fh.read(min(chunk_len, 4096))
                    text = json_data.decode("utf-8", errors="ignore")
                    if "AI3DStudio-Fallback" in text or "FallbackMesh" in text:
                        os.remove(fp)
                        cleaned += 1
            except Exception:
                pass
        if cleaned:
            log_gen("[CACHE] Cleaned " + str(cleaned) + " poisoned fallback cache entries")
    except Exception as e:
        log_error("[CACHE] Cache cleanup error: " + str(e))


# Run cache cleanup at startup
try:
    clean_poisoned_cache()
except Exception:
    pass
...
def run_blender_script(script_text, output_path):
    """Run a Blender script directly (no retry/fix cycle). Used by presets and quick_shape."""
    ...
    try:
        ...
        log_error("[BLENDER-PRESET] Cannot write script: " + str(e))
        return False
    ...
    try:
        ...
        log_gen("[BLENDER-PRESET] exit=" + str(result.returncode))
        if result.stderr:
            error_lines = [l for l in result.stderr.split("\n")
                           if any(kw in l for kw in
                                  ["Error", "Traceback", "TypeError",
                                   "AttributeError", "NameError", "SyntaxError"])]
            for line in error_lines[:5]:
                if line.strip():
                    log_gen("[BLENDER-PRESET] " + line.strip()[:200])

        ok, msg = validate_glb(output_path)
        if ok:
            log_gen("[BLENDER-PRESET] success: " + msg)
            return True
        log_gen("[BLENDER-PRESET] output invalid: " + msg)
        return False
    except subprocess.TimeoutExpired:
        log_gen("[BLENDER-PRESET] timeout after " + str(blender_timeout) + "s")
        return False
    except FileNotFoundError:
        log_gen("[BLENDER-PRESET] Blender not found at " + BLENDER_EXE)
        return False
    except Exception as e:
        log_error("[BLENDER-PRESET] subprocess error: " + str(e))
        return False
...
BLENDER_SYSTEM = (
    ...
    "OUTPUT RAW PYTHON ONLY. NO markdown. NO backticks. NO comments. "
    ...
    "[primitives here] "
    "bpy.ops.export_scene.gltf(filepath=OUTPUT_PATH,export_format='GLB') "
    "OUTPUT_PATH IS ALREADY DEFINED - NEVER define it yourself. "
    "BLENDER 4.2 RULES - CRITICAL: "
    "1. NEVER use align= parameter - it does not exist in 4.2 "
    "2. ALL primitive arguments must be KEYWORD arguments only "
    "3. Every ( must close on the SAME line - no multiline expressions "
    "4. No f-strings. No try/except. No functions. No classes. "
    "5. Each line max 100 characters "
    "CORRECT primitives for Blender 4.2: "
    ...
    "AFTER EVERY PRIMITIVE - these exact 3 lines: "
    "obj = bpy.context.active_object "
    "obj.location = (x, y, z) "
    "obj.scale = (sx, sy, sz) "
    "COLORING every object: "
    "mat = bpy.data.materials.new(name='M') "
    "mat.use_nodes = True "
    "bsdf = mat.node_tree.nodes.get('Principled BSDF') "
    "if bsdf: bsdf.inputs[0].default_value = (R,G,B,1.0) "
    "obj.data.materials.append(mat) "
    "MINIMUM 15 objects. Place each at correct 3D position. "
...
def save_model():
    ...
    user_storage = os.path.join(BASE_DIR, "storage", "users", sub_id)
    folder_dir = os.path.join(user_storage, folder)
    ...
    entry = {
        ...
        "user_id":       sub_id,
        "file":          "storage/users/" + sub_id + "/" + folder + "/" + fname,
        "cloud_url":     cloud_url,
...
def edit_model():
    ...
    t = threading.Thread(
        target=run_generation,
        args=(combined, color_hex, "default", add_list, remove_list, False, "realistic", 3, sub_id),
        daemon=True
    )
    t.start()
    return jsonify({"status": "started"})
...
def regenerate():
    ...
    t = threading.Thread(
        target=run_generation,
        args=(last_prompt, color_hex, "default", [], [], False, "realistic", 3, sub_id),
        daemon=True
    )
    t.start()
